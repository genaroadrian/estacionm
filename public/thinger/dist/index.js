...
var config = JSON.parse(await (0, _promises.readFile)(new URL('./config/config.json', import.meta.url)));
...
async function refrescarConfiguracion() {
  config = JSON.parse(await (0, _promises.readFile)(new URL('./config/config.json', import.meta.url)));
}
...
async function obtenerDatosThinger(ultima_fecha) {
  ...
  const response = await (0, _nodeFetch.default)(`${url}${bucket}${parametros}`, {
    method: 'GET',
    headers: {
...
async function login() {
  ...
  const response = await (0, _nodeFetch.default)(`${url}oauth/token`, {
    method: 'POST',
    headers: {
...
function actualizarConfiguracion(token) {
  ...
  _fs.default.writeFileSync(rutaConfig, JSON.stringify(nueva_config));
}

async function obtenerRegistroMasActual() {
  const response = await (0, _nodeFetch.default)(`${local_url}ultimo_registro`);
  return await response.json();
}
...
function preparDatos(datosThinger) {
  datosThinger.forEach(element => {
    let variable = {
      fecha: (0, _moment.default)(new Date(element.ts)).format('YYYY-MM-DD HH:mm:ss'),
    ...
    guardarRegistro(variable).then(response => {}).catch(error => {});
  });
}

async function guardarRegistro(registroThinger) {
  const response = await (0, _nodeFetch.default)(`${local_url}api/guardar_variable`, {
    method: 'POST',
    headers: {
